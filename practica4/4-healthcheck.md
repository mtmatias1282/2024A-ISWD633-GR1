...
```
Para verificar un proceso específico: Si el contenedor está ejecutando un proceso y quieres comprobar que está en ejecución:
```
--health-cmd="pgrep my_process_name || exit 1"
```
Para comprobar la existencia de un archivo específico: Si tu aplicación crea un archivo al arrancar correctamente:
```
--health-cmd="test -f /path/to/your/file || exit 1"
```
Para verificar un puerto específico: Si necesitas comprobar que un puerto está abierto:
```
